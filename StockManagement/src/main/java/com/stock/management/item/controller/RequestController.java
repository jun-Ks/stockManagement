...
public class RequestController {
	...
	public ResponseEntity<String> insertRequestStockInfo(@RequestBody RequestStockDTO requestInfo){
		...
		if(result != 0) {
			return ResponseEntity.ok(requestInfo + "건 입고 요청 완료");
			
		}else {
			return ResponseEntity.ok("요청실패.. 전산팀에 문의해주세요!");
		}
	}
	...
	public ResponseEntity<String> insertRequestModiLocationInfo(@RequestBody List<RequestModiLocationDTO> requestInfo) {
		...
		return ResponseEntity.ok("요청 등록 실패.. 전산팀에 문의해주세요.");
	}
	...
	@PostMapping("/request/stock/list/approval/{approval}")
	public ResponseEntity<List<RequestStockDTO>> getReqeustStockListByApproval(@PathVariable("approval") int approval){
	...
	//approval 여부 따라 입고 요청 리스트 조회
	@PostMapping("/request/modi-location/list/approval/{approval}")
	public ResponseEntity<List<RequestModiLocationDTO>> getRequestModiLocaListByApproval(@PathVariable("approval") int approval){
	...
	public ResponseEntity<String> approvalStockRequest(@PathVariable("approvalUser") String approvalUser, 
		...
		if(insertResult <= 0) {
			return ResponseEntity.ok("입고정보등록 실패.. 전산팀에 문의 해주세요.");
		}
		...
		if(finalResult > 0) {
			...
			return ResponseEntity.ok("등록실패.. 전산팀에 문의해주세요.");
		}
	}
	...
	public ResponseEntity<String> approvalLocationRequest(
		...
		if(updateResult > 0 ) {
			...
			return ResponseEntity.ok("위치수정실패.. 전산팀에 문의해주세요");
		}
	}
}
